...
pub(crate) fn integrated_acceptance_media_fixture_path(label: &str) -> PathBuf {
...
pub(crate) fn write_integrated_acceptance_test_wav(path: &Path) {
    let channels = 1u16;
...
pub(crate) fn write_g07_acceptance_transient_wav(path: &Path) {
    let channels = 1u16;
    let sample_rate = 48_000u32;
    let bits_per_sample = 16u16;
