//! Device-less clocked soak with the LV2 in-process processor on a Sum
//! stage (g11.033): the render plane runs under
//! `signal_hardware::FakeClockedBackend` for a sustained stretch of
//! callbacks with the rustc-compiled LV2 fixture processing every block.
//!
...
//! (`miss_count == 0` — the ports-stay-connected `run(n)` path kept every
//! deadline), health counters advance, and the fixture's default gain is
//! audible in the stage meters.

use std::sync::Arc;
use std::time::Duration;

use signal_hardware::{FakeClockedBackend, OutputStreamBackend, OutputStreamSpec};
use signal_plugin_bridge::InProcessLv2Processor;
use signal_plugin_lv2::fixture::{compile_lv2_fixture, rustc_available};
...
fn lv2_in_process_soak_processes_every_clocked_block_without_misses() {
    ...
    let plugin_uri = "https://signal.dev/fixtures/lv2/clocked-soak";
    let bundle = compile_lv2_fixture(&directory, plugin_uri, "Signal LV2 Clocked Soak")
        .expect("lv2 fixture should compile");

    let backend = Arc::new(
        InProcessLv2Processor::load_and_activate(
            &bundle,
            plugin_uri,
            SAMPLE_RATE_HZ,
    ...
    assert_eq!(
        backend.miss_count(),
        0,
        "the alloc-free LV2 session must never bypass under clocked load",
    );

    // The insert is audibly live: the fixture's default gain halves the
    // lane feed, so the insert stage meters read below the lane's peak.
