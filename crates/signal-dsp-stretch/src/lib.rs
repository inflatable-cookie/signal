...
impl OfflineHighQualityStretcher {
    ...
    /// Report-only fixed-ratio path with a bounded digital-silence tail anchor.
    ///
    /// The candidate changes only the final 256 output frames and lands the
    /// final sample on digital silence. It is not a promoted
    /// OfflineHighQuality renderer.
    #[doc(hidden)]
