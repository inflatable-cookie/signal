...
fn main() {
    ...
    .expect("local plugin capability scan should succeed");
    ...
    if formats.is_empty() || formats.contains(&PluginFormat::Clap) {
        ...
        for plugin in adapter.discover_plugins_for_roots(&roots) {
            assert!(
                ...
                "local host discovery snapshot should include scanned CLAP type {}",
    ...
    if formats.is_empty() || formats.contains(&PluginFormat::Vst3) {
        ...
        for plugin in adapter.discover_plugins_for_roots(Vst3HostPlatform::MacOs, &roots) {
            assert!(
                discovered_type_ids.contains(&plugin.plugin_type_id.0),
                "local host discovery snapshot should include scanned VST3 type {}",
                plugin.plugin_type_id.0
            );
    ...
    if formats.is_empty() || formats.contains(&PluginFormat::Au) {
        ...
        for plugin in adapter.discover_plugins_for_roots(AuHostPlatform::MacOs, &roots) {
            assert!(
                discovered_type_ids.contains(&plugin.plugin_type_id.0),
                "local host discovery snapshot should include scanned AU type {}",
                plugin.plugin_type_id.0
            );
    ...
    println!(
        ...
        serde_json::to_string_pretty(&inventory)
            .expect("local plugin capability inventory should serialize")
    );
}
