...
pub(super) fn write_vst3_bundle(root: &Path, bundle: &str, plugin_type_id: &str) {
    ...
    .expect("public local vst3 info plist should be written");
    fs::write(
        bundle_root
            .join("Contents")
            .join("Resources")
    ...
    .expect("public local vst3 moduleinfo should be written");
}

pub(super) fn write_clap_fixture_library(
    root: &Path,
    file_name: &str,
    plugin_type_id: &str,
    plugin_name: &str,
    ...
    .expect("public local clap fixture source should be written");
    let output = Command::new("rustc")
        ...
        .arg(&library_path)
        .output()
        .expect("rustc should build public local clap fixture");
    assert!(
        output.status.success(),
        "public local clap fixture compilation should succeed: {}",
...
pub(super) fn write_au_bundle(root: &Path, bundle: &str, plugin_type_id: &str) {
    write_custom_au_bundle(root, bundle, au_metadata_contents(plugin_type_id));
}

pub(super) fn write_custom_au_bundle(root: &Path, bundle: &str, metadata: &str) {
    let bundle_root = root.join(bundle);
    ...
    .expect("public local au info plist should be written");
}
