...
use signal_hardware::{BackendPolicyTier, HardwareBackend};
...
impl LocalRuntimeHost {
    ...
    fn boot_with_fault_recovery(
        ...
        self.runtime
            .set_active_output_device("coreaudio:default-output");
...
fn local_demo_graph_projection() -> GraphProjection {
    GraphProjection {
        graph_id: "signal.host.local.demo".into(),
        node_count: 3,
        nodes: vec![
            GraphNodeProjection {
                node_id: "pre-shaper".into(),
...
fn local_demo_runtime_assembly() -> LocalDemoRuntimeAssembly {
    LocalDemoRuntimeAssembly {
        graph: local_demo_graph_projection(),
        plugin_sandboxes: vec![LocalDemoPluginSandboxAssembly {
            ...
            plugin_format: "clap",
            bound_node_ids: vec!["plugin-insert"],
        }],
    }
}
...
#[cfg(test)]
mod tests {
    use super::{local_demo_runtime_assembly, LifecycleRunSummary, LocalRuntimeHost};
    ...
    fn prepare_local_host_with_lifecycle() -> (
        ...
        let hardware_request = host.coreaudio.default_output_request(
            host.runtime.config().sample_rate.0,
            host.runtime.config().graph.block_size,
        );
        host.runtime
    ...
    fn local_host_rolls_leases_forward_after_timeout() {
        ...
        assert_eq!(supervisor.observation.engine_block_snapshot.phase_count, 3);
        assert_eq!(
            supervisor
                .observation
                .engine_block_snapshot
        ...
        assert!(matches!(
            ...
            Some(signal_runtime::RuntimePreworkRetirementReason::PlanningWindowRevised)
                | Some(signal_runtime::RuntimePreworkRetirementReason::TransportChanged)
        ));
        assert!(supervisor
            .observation
            .engine_block_snapshot
        ...
        assert!(automation.last_value.is_some());
