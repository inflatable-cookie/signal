...
fn runtime_preview_transform_snapshot_derives_from_stretch_and_artifact_baselines() {
    ...
    assert_eq!(
        preview_transform.preview_device_policy.audition_sink_authority,
    ...
    assert_eq!(
        preview_transform.preview_workflow.queued_preview_request_count,
        1
    );
    assert_eq!(preview_transform.preview_workflow.previewable_asset_count, 1);
    assert_eq!(
        preview_transform.preview_workflow.active_audition_clip_count,
        1
    );
    assert_eq!(
        preview_transform.preview_workflow.pending_transform_clip_count,
        0
    );
    assert_eq!(
        preview_transform.preview_workflow.ready_transform_clip_count,
        1
    );
    assert_eq!(
        preview_transform.preview_workflow.fallback_transform_clip_count,
        0
    );
    assert_eq!(
    ...
    assert_eq!(
        observation.preview_transform_snapshot.active_audition_clip_count,
        1
    );
    assert_eq!(
        observation
    ...
    assert_eq!(
        observation.preview_transform_snapshot.preview_workflow.queue_posture,
        RuntimePreviewBrowserQueuePosture::SingleActivePreviewQueue
    );
    assert!(observation
        .render_json()
    ...
    assert_eq!(preview.preview_transform_snapshot.artifact_backed_clip_count, 1);
    assert_eq!(preview.preview_transform_snapshot.active_audition_clip_count, 0);
    assert_eq!(
        preview
            .preview_transform_snapshot
            .preview_device_policy
    ...
    assert_eq!(
        preview.preview_transform_snapshot.preview_workflow.queue_posture,
        RuntimePreviewBrowserQueuePosture::GuardedPreviewQueue
    );
    assert_eq!(
        preview.preview_transform_snapshot.preview_workflow.queue_class,
        RuntimePreviewBrowserQueueClass::PreviewAssetSelectionQueue
    );
    assert_eq!(
        preview.preview_transform_snapshot.preview_workflow.queue_outcome,
