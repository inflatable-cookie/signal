...
fn runtime_transform_artifact_snapshot_derives_from_stretch_and_marker_analysis_baselines() {
    ...
    assert_eq!(
        transform_artifact.transform_persistence.retention_policy_class,
    ...
    assert_eq!(
        transform_artifact.transform_persistence.cache_placement_posture,
        RuntimeTransformCachePlacementPosture::RuntimeCacheRootPlacement
    );
    assert_eq!(
        transform_artifact.transform_persistence.cache_placement_authority,
        RuntimeTransformCachePlacementAuthority::RuntimeDefault
    );
    assert_eq!(
        transform_artifact.transform_persistence.cache_placement_outcome,
        RuntimeTransformCachePlacementOutcome::PreserveRuntimeCacheRoot
    );
    assert_eq!(
        transform_artifact.transform_persistence.persistent_clip_count,
        1
    );
    assert_eq!(
        transform_artifact.transform_persistence.guarded_persistence_clip_count,
        0
    );
    assert_eq!(
        transform_artifact
    ...
    assert!(!transform_artifact.transform_persistence.cache_root_path.is_empty());
    assert_eq!(
    ...
    assert_eq!(observation.transform_artifact_snapshot.reusable_clip_count, 1);
    assert_eq!(
        observation
