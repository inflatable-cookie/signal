...
TEST_CASE("Phase 7 - Subgroup routing structure", "[graph][phase7][routing]") {
    ...
    // Track 1 -> fader-track-1 -> fader-bus -> device
    // Track 2 -> fader-track-2 /
    GraphSnapshot snapshot;
    snapshot.id = "test-subgroup";

    // Nodes
    NodeDesc mixerTrack1;
    mixerTrack1.nodeId = "fader-track-1";
    mixerTrack1.kind = NodeKind::Fader;
    snapshot.nodes.push_back(mixerTrack1);

    NodeDesc mixerTrack2;
    mixerTrack2.nodeId = "fader-track-2";
    mixerTrack2.kind = NodeKind::Fader;
    snapshot.nodes.push_back(mixerTrack2);

    NodeDesc mixerBus;
    mixerBus.nodeId = "fader-bus";
    mixerBus.kind = NodeKind::Fader;
    ...
    // Verify nodes exist
    REQUIRE(engine.findNode("fader-track-1") != nullptr);
    REQUIRE(engine.findNode("fader-track-2") != nullptr);
    REQUIRE(engine.findNode("fader-bus") != nullptr);
