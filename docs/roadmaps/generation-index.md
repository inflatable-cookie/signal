...
| `g10` | 2026-06-11 | Turn the 2026-06-11 deep audit into a remediation program: fix the real audio path, demolish simulated/narration mass (~70-80k LoC), consolidate hygiene, defer rebuilds to demand | Active; `g10.030` retained the frozen stretch baseline and `g10.031` privately admitted fixed-ratio `DirectRenewalDream`; Batch 31.69 froze one complete `LayeredCloud` authority and Batch 31.70 isolated candidate is ready |
...
once in its named isolated worktree. Complete two unchanged conformance rounds
before its acoustic checkpoint, then run `Y01..Y05`. Keep `main`, overlaps,
routing, controls, cache, dynamic ratio, product exposure, Loophole, and Chorus
