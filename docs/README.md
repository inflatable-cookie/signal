...
- Rejected creative ListeningLedSourceRelativeRenewalSpectral brief: [architecture/offline-creative-verified-source-relative-renewal-spectral-brief.md](./architecture/offline-creative-verified-source-relative-renewal-spectral-brief.md)
...
Run `g10.031` Batch 31.35 only. Reconcile the rejected candidate's `Y08`
complete-impulse range with mapped non-zero support and prior passed evidence.
Do not implement DSP. Only drop into `specs/` when a strict lane is explicitly
reopened.
